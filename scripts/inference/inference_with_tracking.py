...
class InferenceTracker:
    ...
    def process_video(self, video_path: str, output_dir: str = None) -> Dict:
        ...
        while True:
            ...
            # Mark if detection was real or predicted (for visibility flag)
            for obj in tracked_objects:
                obj['visible'] = 1 if detection_exists else 0
            
            # Store per-frame data for CSV (requirements format: frame,x,y,visible)
            if tracked_objects:
                for obj in tracked_objects:
                    frame_csv = {
                        'frame': frame_count,
                        'x': obj['centroid'][0],
                        'y': obj['centroid'][1],
                        'visible': obj.get('visible', 1)
                    }
                    csv_data.append(frame_csv)
            else:
                # No tracked objects - add empty row to maintain frame continuity
                frame_csv = {
                    'frame': frame_count,
                    'x': '',
                    'y': '',
